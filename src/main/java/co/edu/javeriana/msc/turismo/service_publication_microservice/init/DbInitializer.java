...
public class DbInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        accommodationTypes.forEach(type -> {
            ...
            var accommodationDB = accommodationTypeRepository.save(accommodationType);
            var serviceType = serviceTypeMapper.toServiceType(accommodationDB);
        ...
        foodTypes.forEach(type -> {
            ...
            var foodDB = foodTypeRepository.save(foodType);
            var serviceType = serviceTypeMapper.toServiceType(foodDB);
            servicesQueueService.sendServiceType(new ServiceTypeDTO(LocalDateTime.now(), CRUDEventType.CREATE, serviceType));
        });
        ...
        transportTypes.forEach(type -> {
            ...
            var transportDB = transportTypeRepository.save(transportType);
            var serviceType = serviceTypeMapper.toServiceType(transportDB);
            servicesQueueService.sendServiceType(new ServiceTypeDTO(LocalDateTime.now(), CRUDEventType.CREATE, serviceType));
        });
        ...
        for (int i = 0; i < 10; i++) { //TODO use API to get real locations
            ...
            var locationDB = locationRepository.save(location);
            var locationDTO = locationMapper.toLocationResponse(locationDB);
            servicesQueueService.sendLocation(new LocationDTO(LocalDateTime.now(), CRUDEventType.CREATE, locationDTO));
        }
